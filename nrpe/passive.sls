...
check_sslyze_{{ formula }}:
  cmd:
    - run
    - name: /usr/lib/nagios/plugins/check_ssl_configuration.py --cronjob -F {{ formula }} -N {{ formula }}_ssl_configuration
    - require:
      - file: check_ssl_configuration.py

test_check_sslyze_{{ formula }}:
  cmd:
    - run
    - name: /usr/lib/nagios/plugins/check_ssl_configuration.py --cronresult -F {{ formula }} -N {{ formula }}_ssl_configuration
    - require:
      - cmd: check_sslyze_{{ formula }}
        {%- endif -%}
    {%- endif -%}
{%- endmacro %}
