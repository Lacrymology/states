...
if 'win' in sys.platform:
    ...
    default_paths = {
        ...
        'etc':     "/usr/local/shinken/etc/shinken",
        'run':     "/var/run/shinken",
        'log':     "/var/log/shinken",
    ...
    data_files = [
        (
            os.path.join('/usr/local/shinken/etc', 'init.d'),
            ['bin/init.d/shinken',
...
if pwd and not root and is_install:
    ...
    _chmodplusx('/usr/local/shinken/etc/init.d/shinken')
    for d in ['scheduler', 'broker', 'receiver', 'reactionner', 'poller', 'arbiter']:
        _chmodplusx('/usr/local/shinken/etc/init.d/shinken-'+d)
